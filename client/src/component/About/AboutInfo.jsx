...
const AboutInfo = () => {
  ...
  return (
    <div className={`flex items-center justify-center min-h-screen ${darkMode ? 'dark:bg-gray-900' : 'bg-gray-100'} p-6`}>
      <Card className={`max-w-3xl w-full ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg rounded-2xl p-6`}>
        <h1 className={`text-center text-4xl font-bold ${darkMode ? 'text-cyan-400' : 'text-cyan-700'} border-b-4 border-cyan-600 pb-2`}>
          About Us
        </h1>
        <CardContent className={`mt-6 ${darkMode ? 'text-gray-300' : 'text-gray-700'} font-medium space-y-4`}>
          <p>
            <TooltipProvider>
              <Tooltip>
          ...
          <p className="flex items-center space-y-2 justify-center mt-6">
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <a
